...
public abstract class BaseData
{
    ...
    protected void WriteListString(byte[] bytes, List<string> list, ref int index)
    {
        if (list == null)
        {
        ...
        {
            WriteString(bytes, list[i], ref index);
        }
    }
    /// <summary>
    /// 根据字节数组 读取整形
    ...
    protected List<string> ReadListString(byte[] bytes, ref int index)
    {
        int count = ReadInt(bytes, ref index);
        List<string> list = new List<string>();
        for (int i = 0; i < count; i++)
        {
            list.Add(ReadString(bytes, ref index));
        }
        return list;
    }
}
